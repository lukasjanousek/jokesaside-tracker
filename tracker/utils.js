...
function calculateCredit(paymentCzk, tiers = null) {
  if (!tiers) return paymentCzk;
  let remaining = paymentCzk;
  let totalCredit = 0;
  for (const tier of tiers) {
    if (remaining <= 0) break;
    const tierWidth = tier.to_czk - tier.from_czk;
    const amountInTier = Math.min(remaining, tierWidth);
    const creditInTier = amountInTier / (1 - tier.discount_pct);
    totalCredit += creditInTier;
    remaining -= amountInTier;
  }
  return totalCredit;
}

function formatCzk(n) { return Math.round(n).toLocaleString('cs-CZ') + ' KÄ'; }
const DPH_RATE = 0.21;
function formatCzkDph(n) { return formatCzk(n) + ' bez DPH'; }
function dphAmount(n) { return n * DPH_RATE; }
function withDph(n) { return n * (1 + DPH_RATE); }
...
function getMonthLabel(key) {
  const [y, m] = key.split('-');
  const months = ['Leden','Ãnor','BÅezen','Duben','KvÄten','Äerven','Äervenec','Srpen','ZÃ¡ÅÃ­','ÅÃ­jen','Listopad','Prosinec'];
...
const Icons = {
  ...
  invoice: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>,
...
// Usage: const { data, error } = await supabaseRetry(() => supabase.from('table').select('*'));
window.supabaseRetry = async function(fn, maxRetries = 3) {
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      const result = await fn();
      // If Supabase returned an error object (not a throw), check if retryable
      if (result && result.error) {
        const msg = (result.error.message || '').toLowerCase();
        const isRetryable = msg.includes('fetch') || msg.includes('network') || msg.includes('timeout') || msg.includes('socket') || result.error.code === 'PGRST301';
        if (isRetryable && attempt < maxRetries) {
          const delay = Math.min(1000 * Math.pow(2, attempt), 8000);
          console.warn('Supabase retry ' + (attempt + 1) + '/' + maxRetries + ' after ' + delay + 'ms:', msg);
      ...
      if (attempt < maxRetries) {
        const delay = Math.min(1000 * Math.pow(2, attempt), 8000);
        console.warn('Supabase retry ' + (attempt + 1) + '/' + maxRetries + ' after ' + delay + 'ms:', err.message);
        await new Promise(r => setTimeout(r, delay));
      } else {
        throw err;
      }
